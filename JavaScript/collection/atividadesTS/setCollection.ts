...
const numerosSet: Set<number> = new Set([12, 45, 67, 23, 89, 34, 56, 78, 90, 11]);

function buscarNumeroNoSet(): void {
    const input = readline.question("Digite o número que deseja buscar: ");
    const numeroBusca = parseInt(input, 10);

    if (isNaN(numeroBusca)) {
        console.log("Por favor, digite um número inteiro válido!");
        return buscarNumeroNoSet();
    }
